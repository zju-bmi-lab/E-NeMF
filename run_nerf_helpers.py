...
def event2loss(dimg, event_pos, event_neg, B_pos, B_neg, M=None):
    f_event = event_pos * B_pos - event_neg * B_neg
    if M == None:
        return torch.mean(F.relu(torch.abs(dimg - f_event)-B_pos))
    else:
        return torch.sum(F.relu(torch.abs(dimg - f_event)-B_neg) * M) / (torch.sum(M) + 1e-8)# / x.shape[-1]
...
class NeRF_d(nn.Module):
    ...
    def forward(self, x):
        ...
        control_p = torch.tanh(self.cp_linear(h))
        mid_control_p = nn.Parameter(torch.zeros_like(control_p[..., :3], dtype=torch.float32), requires_grad=False)
        control_p = torch.cat([control_p[..., :9], mid_control_p, control_p[..., 9:]], dim=-1)
        ...
        return torch.cat([outputs, control_p, blending], dim=-1)
...
def create_nerf(args):
    ...
    if args.ft_path is not None and args.ft_path != 'None':
        ...
        ckpts = [os.path.join(basedir, expname, f) for f in sorted(os.listdir(os.path.join(basedir, expname))) if 'tar' in f and 'Pretrained' not in f]
