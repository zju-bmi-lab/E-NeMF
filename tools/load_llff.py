...
from tools.flow_utils import resize_flow
...
def _load_data(basedir, factor=None, width=None, height=None, load_imgs=True, event_pose=False):
    print('factor ', factor)
    ...
    print(event_pose)
    if event_pose:
        poses_arr = np.load(os.path.join(basedir, 'e_poses_bounds.npy'))
        poses = poses_arr[:, :-2].reshape([-1, 3, 5]).transpose([1,2,0])
        bds = poses_arr[:, -2:].transpose([1,0])
...
def load_llff_data(args, basedir,
                   ...
                   frame2dolly=10, event_pose=False):

    poses, bds, imgs, disp, masks, flows_f, flow_masks_f, flows_b, flow_masks_b = \
        _load_data(basedir, factor=factor, event_pose=event_pose) # factor=2 downsamples original imgs by 2x
    ...
    grids = get_grid(int(H), int(W), len(images), flows_f, flow_masks_f, flows_b, flow_masks_b) # [N, H, W, 8]
